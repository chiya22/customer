...
const findForSelect = function (callback) {
    (async function () {
        await connection.query('select id, kubun_company, id_nyukyo, name, ymd_nyukyo, ymd_kaiyaku from companies where ymd_end = "99991231" order by id_nyukyo asc ', function (error, results, fields) {
            if (error) {
                callback(error, null);
            } else {
                callback(null, results);
