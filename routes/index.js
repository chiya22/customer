...
router.post('/', security.authorize(), function (req, res, next) {
  ...
  company.selectSQL(query, (err, retObj) => {
    ...
    const query2 = '(select "会社" AS header, case c.ymd_kaiyaku when "99991231" then "" else "解約" end as kaiyakuKubun, c.id, c.id_nyukyo as id_nyukyo, c.kubun_company as kubun, c.name, c.kana, c.ymd_nyukyo, c.ymd_kaiyaku from companies as c where ((c.name like "%' + searchvalue + '%") or (c.name_other like "%' + searchvalue + '%") or (c.kana like "%' + searchvalue + '%") or (c.id_nyukyo like "%' + searchvalue + '%")) and c.ymd_end = "99991231") union all (select "個人" AS header, case p.ymd_kaiyaku when "99991231" then "" else "解約" end as kaiyakuKubun, p.id, NULL as id_nyukyo, p.kubun_person as kubun, p.name, p.kana, p.ymd_nyukyo, p.ymd_kaiyaku from persons as p where ((p.name like "%' + searchvalue + '%") or (p.kana like "%' + searchvalue + '%")) and p.ymd_end = "99991231") order by kaiyakuKubun asc, header asc, id_nyukyo asc, ymd_kaiyaku desc, ymd_nyukyo asc limit ' + count_perpage + ' offset ' + offset
